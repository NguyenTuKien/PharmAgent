...
- Tổng số handler mappings hiện tại: ~65 (mỗi HTTP method + path được tính riêng). README này liệt kê các endpoint theo module để dễ đọc.
 ...
 | | POST | `/api/auth/profiles/{profileId}/select` | Chọn profile để lấy accessToken (profile-scoped). | **Header:** `Bearer <AuthToken>` | `{\"accessToken\": \"eyJ...\"}` | Auth |
...
|   | 
| **2. Quản lý Profile (Profiles)** | GET | `/api/profiles` | Lấy danh sách profile trong tài khoản. | Header: `Bearer <AccessToken>` | Paginated list | Auth |
 | | GET | `/api/profiles/me` | Lấy profile hiện tại. | Header: `Bearer <AccessToken>` | `{\"profileId\":\"...\",\"firstName\":\"...\",\"contacts\":[],\"devices\":[]}` | Auth |
 | | PUT | `/api/profiles/me` | Cập nhật profile hiện tại. | `{\"firstName\":\"...\",...}` | Updated profile | Auth |
 | | GET | `/api/profiles/me/contacts` | Lấy danh sách user contacts của profile. | Header: `Bearer <AccessToken>` | `[{\"id\":\"c1\",\"name\":\"Bác sĩ A\",\"phone\":\"090111222\"}]` | Auth |
...
||
 | **3. Thiết bị (Devices)** | GET | `/api/profiles/me/devices` | Lấy danh sách device token của profile. | Header: `Bearer <AccessToken>` | `[{\"deviceId\":\"d1\",\"deviceName\":\"iPhone 15\",\"type\":\"MOBILE\"}]` | Auth |
...
||
 | **4. Kết nối Người thân (Relationships)** | GET | `/api/caregiver/relationship` | CAREGIVER: lấy danh sách elderly đang theo dõi. | Header: `Bearer <AccessToken>` | List | CAREGIVER |
