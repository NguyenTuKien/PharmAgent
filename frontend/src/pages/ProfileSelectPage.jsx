...
const roleConfig = {
  ...
  CAREGIVER: {
    ...
    description: 'Theo dõi lịch thuốc, báo cáo và hồ sơ người thân.',
    Icon: Users,
  ...
  ELDERLY: {
    label: 'Người thân',
    description: 'Xem lịch uống thuốc, nhắc nhở và thông tin chăm sóc cá nhân.',
    Icon: HeartPulse,
...
function ProfileAvatar({ profile }) {
  ...
  return (
    <div
      className={`grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-2xl bg-gradient-to-br text-lg font-black shadow-[0_16px_30px_-18px_rgba(22,33,31,0.55)] sm:h-[72px] sm:w-[72px] ${config.avatarClass}`}
    >
      {profile?.avatarUrl ? (
...
function ProfileCard({ disabled, isSelected, onSelect, profile, style }) {
  ...
  return (
    <button
      type="button"
      className="profile-select-card group text-left"
      style={style}
      disabled={disabled}
    ...
    >
      <div className="relative z-[1] flex h-full flex-col gap-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <span
            className={`inline-flex max-w-full items-center gap-2 rounded-full border px-3 py-1 text-xs font-extrabold leading-none ${config.badgeClass}`}
          >
        ...
        <div className="min-w-0">
          <h2 className="text-xl font-black leading-tight tracking-normal text-[var(--text)]">
            {profileName(profile)}
          </h2>
        ...
        <div className="mt-auto flex flex-col gap-4 border-t border-[color:var(--border)] pt-4">
          ...
          <div className="flex items-center justify-between gap-3 text-sm font-black text-[var(--accent-strong)]">
  ...
  return (
    <main className="profile-select-page relative isolate min-h-[100dvh] overflow-hidden px-4 py-6 text-[var(--text)] sm:px-6 lg:px-10">
      <section className="mx-auto flex min-h-[calc(100dvh-3rem)] w-full max-w-4xl items-center justify-center">
        <div className="w-full rounded-[2rem] border border-[color:var(--border)] bg-white/80 p-6 shadow-[0_30px_80px_-45px_rgba(22,33,31,0.45)] backdrop-blur md:p-9">
...
export function ProfileSelectPage() {
  ...
  return (
    <main className="profile-select-page relative isolate min-h-[100dvh] overflow-hidden px-4 py-6 text-[var(--text)] sm:px-6 lg:px-10">
      <section className="mx-auto grid min-h-[calc(100dvh-3rem)] w-full max-w-7xl items-center gap-10 py-8 lg:grid-cols-[0.86fr_1.14fr] lg:py-12">
        <header className="max-w-2xl">
          <div className="brand brand--lockup mb-8">
            <img className="brand-logo" src={logo} alt="" />
            <img className="brand-title" src={title} alt="PharmAgent" />
          </div>

          <p className="eyebrow">Chọn hồ sơ</p>
          <h1 className="mt-3 text-4xl font-black leading-[1.05] tracking-normal text-[var(--text)] sm:text-5xl">
            Bạn muốn tiếp tục với hồ sơ nào?
          </h1>
          <p className="mt-5 max-w-xl text-base leading-7 text-[var(--muted)]">
            Tài khoản người chăm sóc có thể chuyển giữa hồ sơ của chính mình và người thân đã được liên kết. Mỗi hồ sơ giữ quyền truy cập riêng để bảo vệ dữ liệu thuốc.
          </p>

          <div className="mt-8 grid max-w-xl grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-[color:var(--border)] bg-white/70 p-4 backdrop-blur">
              <p className="text-xs font-extrabold uppercase text-[var(--muted)]">Số hồ sơ</p>
              <p className="mt-2 text-2xl font-black text-[var(--accent-strong)]">{sortedProfiles.length}</p>
            </div>
            <div className="rounded-2xl border border-[color:var(--border)] bg-white/70 p-4 backdrop-blur">
              <p className="text-xs font-extrabold uppercase text-[var(--muted)]">Bảo mật</p>
              <p className="mt-2 text-sm font-bold leading-6 text-[var(--text)]">Quyền truy cập theo từng vai trò</p>
            </div>
          </div>
        </header>

        <section className="min-w-0" aria-label="Danh sách hồ sơ">
          {sortedProfiles.length ? (
            ...
            <div className="rounded-[2rem] border border-[color:var(--border)] bg-white/80 p-6 shadow-[0_30px_80px_-45px_rgba(22,33,31,0.45)] backdrop-blur sm:p-8">
          ...
          <div className="mt-6 flex justify-start">
            <button
              type="button"
              className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[color:var(--border)] bg-white/75 px-4 text-sm font-extrabold text-[var(--text)] shadow-[0_12px_30px_-24px_rgba(22,33,31,0.5)] transition duration-200 hover:-translate-y-0.5 hover:border-[#b8d8cf] active:translate-y-0 disabled:opacity-60"
